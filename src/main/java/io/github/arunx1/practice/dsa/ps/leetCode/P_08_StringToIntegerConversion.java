package io.github.arunx1.practice.dsa.ps.leetCode;
 ...
 */
public class P_08_StringToIntegerConversion {
    public int myAtoi(String s) {
        int result = 0;
        boolean resultNegative = false;
        boolean firstDigitProcessed = false;
        boolean symbolProcessed = false;
        for (int i = 0; i < s.length(); i++) {
            char currentChar = s.charAt(i);
            if (!firstDigitProcessed) {
                if (!symbolProcessed && currentChar == ' ')
                    continue;
                else if (!symbolProcessed && (currentChar == '-' || currentChar == '+')) {
                    if (currentChar == '-')
                        resultNegative = true;
            ...
            if (currentChar >= '0' && currentChar <= '9') {
                ...
                int tail = Character.getNumericValue(currentChar);
                if ((Integer.MAX_VALUE - tail) / 10 >= result)
                    result = result * 10 + tail;
                else if (resultNegative)
        ...
        return resultNegative ? result * -1 : result;
    }
}
